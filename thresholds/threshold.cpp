...
int main() {
  std::vector<std::pair<double,double>> vkRatios = {
    std::make_pair(10000.0,1.0),
    std::make_pair(50000.0,1.0),
    std::make_pair(100000.0,1.0)
  ...
  for (std::pair<double,double> vk : vkRatios) {
    double v = vk.first;
    double k = vk.second;
    int N_star = v / (10*k) ;
    std::ofstream myFile("finalAlt/valuecost" + std::to_string((int64_t)(round(v))) + ".txt",std::ofstream::app);

    std::vector<int> BValues = {1,2,3,4,5,10,20,30,40,50,100,150,200,500,1000,2000,3000,4000,5000,6000,7000,1000};
    for (int B = 1; B < N_star; B++) {
      if (B >= N_star) break;
